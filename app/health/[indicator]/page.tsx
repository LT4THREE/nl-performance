...
export async function generateStaticParams() {
  return healthIndicators.map((i) => ({ indicator: i.id }));
}
...
export async function generateMetadata({
  ...
  const indicator = findHealthIndicator(id);
  ...
  try {
    ...
    return indicatorMetadata(indicator, latest, isRelative ? yoyDelta : yoyDeltaPct, `/health/${id}`);
  } catch {
    return indicatorMetadata(indicator, null, null, `/health/${id}`);
  }
}

export default async function HealthIndicatorPage({
  params,
  searchParams,
}: {
  params: Promise<{ indicator: string }>;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 space-y-10">
      ...
      <div className="text-sm text-[var(--color-muted)]">
        <Link href="/health" className="underline hover:text-[var(--color-fg)]">
          ← Health
        </Link>
      </div>
