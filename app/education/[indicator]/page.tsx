...
import { educationIndicators, findEducationIndicator } from "@/data/indicators/education";
...
export async function generateStaticParams() {
  return educationIndicators.map((i) => ({ indicator: i.id }));
}
...
export async function generateMetadata({
  ...
  const indicator = findEducationIndicator(id);
  ...
  try {
    ...
    return indicatorMetadata(indicator, latest, isRelative ? yoyDelta : yoyDeltaPct, `/education/${id}`);
  } catch {
    return indicatorMetadata(indicator, null, null, `/education/${id}`);
  }
}

export default async function EducationIndicatorPage({
  params,
  searchParams,
}: {
  params: Promise<{ indicator: string }>;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 space-y-10">
      ...
      <div className="text-sm text-[var(--color-muted)]">
        <Link href="/education" className="underline hover:text-[var(--color-fg)]">
          ← Education
        </Link>
      </div>
      ...
      {fetchError && (
        <div className="p-4 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] text-sm">
          Could not load data right now. ({fetchError})
        </div>
      )}
